...
const createJsonObj = (userInputInfo) => {
  ...
  const createObj = (env) => ({
    name: userInputInfo[`${env}Name`],
      ...
      maxBackupVersionCount: userInputInfo[`${env}MaxBackupVersionCount`],
      isRemoveRemoteFile: userInputInfo[`${env}IsRemoveRemoteFile`],
      isRemoveLocalFile: userInputInfo[`${env}IsRemoveLocalFile`],
      isAll: userInputInfo[`${env}IsAll`],
      exclude: [],
      webPath: userInputInfo[`${env}WebPath`] || '',
      isRemoteDeploy: userInputInfo[`${env}IsRemoteDeploy`],
      cloneScript: userInputInfo[`${env}CloneScript`] || '',
      webProjectPath: userInputInfo[`${env}WebProjectPath`] || '',
      downloadDirName: userInputInfo[`${env}DownloadDirName`] || '',
      install: userInputInfo[`${env}Install`] || '',
      startRemoteProgress: userInputInfo[`${env}StartRemoteProgress`] || ''
  })
