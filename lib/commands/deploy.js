...
const batchBuildZip = async (config, index) => {
    ...
    let dirNames = fs.readdirSync(path.resolve(process.cwd())).filter(c => c !== 'node_modules' && c !== '.git' && c !== '.idea' && c.indexOf('.') === -1 && c !== 'LICENSE').concat([config.distPath])
...
// 上传本地可选文件
const batchUploadAttachLocalFile = async (config, index) => {
  for(let i = 0; i < config.attachUploadFiles.length; i++) {
    const item = config.attachUploadFiles[i];
    const j = i + 1;
    try {
...
const createTaskList = (config) => {
  ...
  if(isAll) {
    ...
    attachUploadFiles && taskList.push(batchUploadAttachLocalFile)
    ...
    script && taskList.push(execRemoatInstall)
  } else {
